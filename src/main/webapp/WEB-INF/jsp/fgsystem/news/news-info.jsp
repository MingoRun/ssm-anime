...
<div class="page">
	...
	<nav class="breadcrumb"><!--当前页面所在位置-->
		现在位置<i class="fa fa-chevron-right" aria-hidden="true"></i><a href="${pageContext.request.contextPath }/toFrontIndex.action">首页</a><i class="fa fa-chevron-right" aria-hidden="true"></i><a href="${pageContext.request.contextPath }/toFrontNewsList.action">动漫新闻</a><i class="fa fa-chevron-right" aria-hidden="true"></i><a href="${pageContext.request.contextPath }/toFrontNewsCategory.action?newsCategoryId=${news.getNewsCategory().getId() }">${news.getNewsCategory().getName() }</a><i class="fa fa-chevron-right" aria-hidden="true"></i>${news.newsTitle }			
	</nav><!--/当前页面所在位置-->
	<div class="content"><!--内容-->
		<div id="primary"><!--正文内容-->
			<div class="info">
				...
				<div class="info-news">
					<span class="news-time"><i class="fa fa-clock-o" aria-hidden="true"></i> 发布时间：${news.newsReleaseTime }</span>
					<span class="news-source"><i class="fa fa-tachometer" aria-hidden="true"></i> 来源：${news.newsSource }</span>
					<span class="news-category"><i class="fa fa-bars" aria-hidden="true"></i> 类型：${news.getNewsCategory().getName() }</span>
					<span class="news-author"><i class="fa fa-user-o" aria-hidden="true"></i> 作者：${news.newsAuthor }</span>
				</div>
				<div class="info-img">
...
<script>
	$(function(){
		$("#comment-content").emoji({
		    showTab: true,
		    animation: 'fade',
		    basePath: 'resources/images/emoji',
	...
	function showComment(){
		...
		$.ajax({
			type:"GET",
			url:"${pageContext.request.contextPath}/showFrontNewsCommentAll.action",
			data:{"pageNum":1,"pageSize":5,"newsId":newsId},
			dataType:"json",
			success:function(data){ 
			    ...
			    jump:function(obj,first){  //jump - 切换分页的回调
			        ...
			        if(!first){
			    		...
			    		$.ajax({
			    			type:"GET",
			    			url:"${pageContext.request.contextPath}/showFrontNewsCommentAll.action",
			    			data:{"pageNum":obj.curr,"pageSize":obj.limit,"newsId":newsId},
			    			dataType:"json",
			    			success:function(data){ 
			    				console.log(data.comments);
			    				var html = "";
	...
	$("#comment-btn").click(function(){
		if(bool==false){
			...
			$.ajax({
				type:"POST",
				url:"${pageContext.request.contextPath}/addNewsComment.action",
				data:{"content":content,"newsId":newsId,"userId":userId},
				dataType:"text",
				success:function(data){
					if(data=="OK"){
						$("#comment-content").val("");
