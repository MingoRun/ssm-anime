...
<div class="page">
	...
	<nav class="breadcrumb"><!--当前页面所在位置-->
		现在位置<i class="fa fa-chevron-right" aria-hidden="true"></i><a href="${pageContext.request.contextPath }/toFrontIndex.action">首页</a><i class="fa fa-chevron-right" aria-hidden="true"></i><a href="${pageContext.request.contextPath }/toFrontComicsList.action">漫画</a><i class="fa fa-chevron-right" aria-hidden="true"></i><c:forEach items="${categoryList }" var="category"><c:if test="${category.comicsCategoryId==comicsCategoryId }">${category.comicsCategoryName }</c:if></c:forEach>			
	</nav><!--/当前页面所在位置-->
	<div class="content"><!--内容-->
		<div class="category"><!--分类-->
			<c:forEach items="${categoryList }" var="category">
				<div class="category-list">
					<a href="${pageContext.request.contextPath }/toFrontComicsCategory.action?comicsCategoryId=${category.comicsCategoryId}">${category.comicsCategoryName }</a>
				</div>
				<div class="space"></div>
			</c:forEach>
			<div class="clear"></div>
				...
				<c:forEach items="${comicsPageInfo.list }" var="comics">
					<div class="comics">
						<div class="comics-cover-img">
							...
							<span class="comics-category"><a href="${pageContext.request.contextPath }/toFrontComicsCategory.action?comicsCategoryId=${comics.getComicsCategory().getComicsCategoryId()}">${comics.getComicsCategory().getComicsCategoryName() }</a></span>
						</div>
...
<script>
	layui.use(['laypage','layer'],function(){
		...
		laypage.render({            
		    ...
		    count: ${comicsPageInfo.total},//数据总数，从服务端得到
		    ...
		    jump:function(obj,first){  //jump - 切换分页的回调
		        ...
		        if(!first){
		          ...
		          //var conf ={"pageNum":obj.curr,"pageSize":obj.limit};
		        	window.location="${pageContext.request.contextPath }/toFrontComicsCategory.action?pageNum="+obj.curr+"&comicsCategoryId="+${comicsCategoryId};
		        }
		    }
		  });
	});
