        			...
        			<c:forEach items="${comments.list }" var="comment">
        				<tr>
				           <td>
				            ...
				            <td>${comment.id }</td>
				            <td>${comment.getChapter().getComics().getComicsName() }</td>
				            <td>${comment.getChapter().getChapterName() }</td>
    	   		...
    	   		laypage.render({            
    	   		    ...
    	   		    jump:function(obj,first){  //jump - 切换分页的回调
    	   		        ...
    	   		        if(!first){
    	   		          ...
    	   		          window.location="${pageContext.request.contextPath }/showChapterCommentAll.action?pageNum="+obj.curr;
    	   		        }
    	   		    }
    	   		  });
       		});
    			  ...
    			  $.post("${pageContext.request.contextPath}/delCommentById.action",{"id":id},function(data){
    				  if(data == "OK"){
    					  layer.msg('删除成功!',{icon:1,time:1000},function(){
    						  window.location="${pageContext.request.contextPath}/showChapterCommentAll.action?pageNum="+1;
    					  });
    				  }else{
